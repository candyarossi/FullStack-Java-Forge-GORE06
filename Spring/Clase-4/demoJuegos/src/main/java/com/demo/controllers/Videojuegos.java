...
public class Videojuegos implements ManejoDeFechas {
	...
	public Videojuegos() {
		 ...
		 * ArrayList(Arrays.asList("Adventure", "Action")), new
		 * ArrayList(Arrays.asList("PC", "PlayStation"))); Videojuego v2 = new
		 * Videojuego("Outlast",
		 ...
		 * , "", formatearFecha("2013-09-03"), 4.5, new ArrayList(Arrays.asList("Indie",
		 * "Action")), new ArrayList(Arrays.asList("PC", "PlayStation", "Xbox",
		 * "Nintendo"))); Videojuego v3 = new Videojuego("Resident Evil 2",
		 * "The story follows the survivors of a zombie virus outbreak in the fictional Raccoon City."
		 * , "", formatearFecha("2019-01-25"), 4.3, new
		 * ArrayList(Arrays.asList("Shooter", "Action", "Adventure")), new
		 * ArrayList(Arrays.asList("PC", "PlayStation", "Xbox", "Nintendo")));
		 * 
		 * this.videojuegos.add(v1); this.videojuegos.add(v2); this.videojuegos.add(v3);
		 */

		// TODO traer juegos de la API
		try {
			this.videojuegos = InfoApi.get10Videogames(1);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	@PostMapping("/guardar")
	public String guardar(@RequestParam String nombre, @RequestParam String descripcion, @RequestParam String portada,
			@RequestParam String fecha_lanzamiento, @RequestParam String rating, @RequestParam String generos,
			@RequestParam String plataformas) {

		// TODO recibir un id y fecha como string
		Videojuego videojuego = new Videojuego(0, nombre, descripcion, portada, fecha_lanzamiento,
				Double.parseDouble(rating), new ArrayList<String>(Arrays.asList(generos)),
	...
	/*
	 * @GetMapping("/detalle/{nombre}") public String
	 * detalle(@PathVariable("nombre") String nombre, Model modelo) { Videojuego
	 * videojuego = buscar(nombre); modelo.addAttribute("videojuego", videojuego);
	 * return "detalleVideojuego.jsp"; }
	 */

	// TODO crear ruta para buscar por id (cambiar en jsp)
	@GetMapping("/detalle/{id}")
	public String detalle2(@PathVariable("id") int id, Model modelo) {
