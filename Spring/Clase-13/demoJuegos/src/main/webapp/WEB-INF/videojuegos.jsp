...
<body>
	<div class="nav">
		...
		<c:if test="${miVideojuego == null}">		
			<a href="/form/agregar">Crear Juego</a>
		</c:if>
	...
	<ul class="videojuegos">
		<c:forEach var="juego" items="${videojuegos}">
			<li>
				...
				<h3>${juego.nombre}</h3>
				<a href="/detalle/${juego.id}?fuente=api">Detalles</a>
			</li>
		</c:forEach>
	</ul>
	</div>
	...
	<c:if test="${miVideojuego != null}">
		...
		<ul class="videojuegos">
			...
			<a href="/detalle/${miVideojuego.id}?fuente=db">Detalles</a>
			<a href="/form/editar/${miVideojuego.id}">Editar</a>
			<form action="/eliminar/${miVideojuego.id}" method="POST">
